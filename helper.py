...
def correlation_screen(
    ...
    for col in X.columns:
        ...
        for method in methods:
            if len(df) < 3 or df["x"].nunique() < 2 or df["y"].nunique() < 2:
                ...
                result = corr_fn[method](df["x"], df["y"])
                corr, p_value = float(result.statistic), float(result.pvalue)
            row[f"{method}_corr"] = corr
            row[f"{method}_abs_corr"] = abs(corr) if pd.notna(corr) else np.nan
    ...
    for method in methods:
        out[f"{method}_q_value"], out[f"{method}_signif"] = _fdr(out[f"{method}_p_value"], fdr_method)
...
def joint_ols_report(
    ...
    _print_header("Joint OLS")
    print(model.summary())

    _print_header(f"Joint OLS ({robust})")
